...
"""Test module for :mod:`core.models.EMailCorrespondents`."""
...
def test_EMailCorrespondents_fields(fake_emailCorrespondent):
...
@pytest.mark.django_db
def test_EMailCorrespondents___str__(fake_emailCorrespondent):
    """Tests the string representation of :class:`core.models.EMailCorrespondents.EMailCorrespondents`."""
...
@pytest.mark.django_db
def test_EMailCorrespondents_foreign_key_email_deletion(fake_emailCorrespondent):
    """Tests the on_delete foreign key constraint on email in :class:`core.models.EMailCorrespondents.EMailCorrespondents`."""
    fake_emailCorrespondent.email.delete()

    with pytest.raises(EMailCorrespondents.DoesNotExist):
...
@pytest.mark.django_db
def test_EMailCorrespondents_foreign_key_correspondent_deletion(
    fake_emailCorrespondent,
):
    """Tests the on_delete foreign key constraint on correspondent in :class:`core.models.EMailCorrespondents.EMailCorrespondents`."""
...
@pytest.mark.django_db
def test_EMailCorrespondents_unique_constraints(fake_emailCorrespondent):
...
def test_EMailCorrespondents_createFromHeader_success(
    fake_email, fake_header_name, header, expectedResults
):
    """Tests :func:`core.models.EMailCorrespondents.EMailCorrespondents.createFromHeader`
    in case of success.
    """
    assert EMailCorrespondents.objects.count() == 0
    assert Correspondent.objects.count() == 0

    result = EMailCorrespondents.createFromHeader(header, fake_header_name, fake_email)

    assert EMailCorrespondents.objects.count() == len(expectedResults)
    ...
    for item, expectedResult in zip(result, expectedResults, strict=True):
        assert isinstance(item, EMailCorrespondents)
...
@pytest.mark.django_db
def test_EMailCorrespondents_createFromHeader_no_correspondent(
    ...
    result = EMailCorrespondents.createFromHeader("", fake_header_name, fake_email)
...
@pytest.mark.django_db
def test_EMailCorrespondents_createFromHeader_no_address(fake_email, fake_header_name):
    """Tests :func:`core.models.EMailCorrespondents.EMailCorrespondents.createFromHeader`
    in case of the correspondent cannot be set up.
    """
    assert EMailCorrespondents.objects.count() == 0
...
@pytest.mark.django_db
def test_EMailCorrespondents_createFromHeader_no_email(fake_header_name, faker):
    """Tests :func:`core.models.EMailCorrespondents.EMailCorrespondents.createFromHeader`
