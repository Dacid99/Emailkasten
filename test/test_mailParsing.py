...
@pytest.mark.parametrize(
    ...
    [
        (
            ["Test äöüß§ <test@testdomain.tld>"],
            [("Test äöüß§", "test@testdomain.tld")],
            False,
        ),
        (
            ["Test Persön <testtestdomain.tld>"],
            [("Test Persön", "testtestdomain.tld")],
            True,
        ),
...
def test__separateRFC2822MailAddressFormat(
    mock_logger, testMailers, expectedResult, warningCalled
):
    separatedMailers = core.utils.mailParsing.separateRFC2822MailAddressFormat(
        testMailers
    )
...
def test__parseMessageID_success(
    mock_logger, mock_good_mailMessage, mock_empty_parsedMailDict
):
    core.utils.mailParsing._parseMessageID(
        mock_good_mailMessage, mock_empty_parsedMailDict
    )
    assert core.constants.ParsedMailKeys.Header.MESSAGE_ID in mock_empty_parsedMailDict
    assert (
        mock_empty_parsedMailDict[core.constants.ParsedMailKeys.Header.MESSAGE_ID]
        == "abcdefgäöüß§"
    )
...
def test__parseMessageID_emptyMessage(
    mock_logger, mock_bad_mailMessage, mock_empty_parsedMailDict
):
    core.utils.mailParsing._parseMessageID(
        mock_bad_mailMessage, mock_empty_parsedMailDict
    )
    assert core.constants.ParsedMailKeys.Header.MESSAGE_ID in mock_empty_parsedMailDict
    assert mock_empty_parsedMailDict[
        core.constants.ParsedMailKeys.Header.MESSAGE_ID
    ] == str(hash(mock_bad_mailMessage))
    mock_logger.debug.assert_called()
...
def test__parseMessageID_noMessage(mock_no_mailMessage, mock_empty_parsedMailDict):
    with pytest.raises(AttributeError):
        core.utils.mailParsing._parseMessageID(
            mock_no_mailMessage, mock_empty_parsedMailDict
        )


def test__parseDate_success(
    mock_logger, mock_good_mailMessage, mock_empty_parsedMailDict
):
    ...
    assert mock_empty_parsedMailDict[
        core.constants.ParsedMailKeys.Header.DATE
...
def test__parseDate_emptyMessage(
    mock_logger, mock_bad_mailMessage, mock_empty_parsedMailDict
):
...
def test__parseDate_noMessage(mock_no_mailMessage, mock_empty_parsedMailDict):
    with pytest.raises(AttributeError):
        core.utils.mailParsing._parseDate(
            mock_no_mailMessage, mock_empty_parsedMailDict
        )
...
@pytest.mark.parametrize(
    ...
    [
        (True, "This a test SUBJEcT line äöüß§"),
        (False, "This a test SUBJEcT line äöüß§ \t "),
    ],
)
def test__parseSubject_success(
    mock_logger,
    stripTexts,
    expectedResult,
    override_config,
    mock_special_mailMessage,
    mock_empty_parsedMailDict,
):
    with override_config(STRIP_TEXTS=stripTexts):
        core.utils.mailParsing._parseSubject(
            mock_special_mailMessage, mock_empty_parsedMailDict
        )

    assert core.constants.ParsedMailKeys.Header.SUBJECT in mock_empty_parsedMailDict
    assert (
        mock_empty_parsedMailDict[core.constants.ParsedMailKeys.Header.SUBJECT]
        == expectedResult
    )
    mock_logger.debug.assert_called()
    mock_logger.warning.assert_not_called()
...
def test__parseSubject_emptyMessage(
    mock_logger, mock_bad_mailMessage, mock_empty_parsedMailDict
):
    core.utils.mailParsing._parseSubject(
        mock_bad_mailMessage, mock_empty_parsedMailDict
...
def test__parseHeader_success(
    mock_logger, mock_good_mailMessage, mock_empty_parsedMailDict
):
    core.utils.mailParsing._parseHeader(
        mock_good_mailMessage, "test", mock_empty_parsedMailDict
    )
...
def test__parseHeader_emptyMessage(
    mock_logger, mock_bad_mailMessage, mock_empty_parsedMailDict
):
    core.utils.mailParsing._parseHeader(
        mock_bad_mailMessage, "test", mock_empty_parsedMailDict
    )
    assert "test" in mock_empty_parsedMailDict
...
def test__parseHeader_noMessage(mock_no_mailMessage, mock_empty_parsedMailDict):
    with pytest.raises(AttributeError):
        core.utils.mailParsing._parseHeader(
            mock_no_mailMessage, "test", mock_empty_parsedMailDict
        )


def test__parseMultipleHeader_success(
    mock_logger, mock_good_mailMessage, mock_empty_parsedMailDict
):
    core.utils.mailParsing._parseMultipleHeader(
        mock_good_mailMessage, "multi", mock_empty_parsedMailDict
    )
...
def test__parseMultipleHeader_noneMulti(
    mock_logger, mock_good_mailMessage, mock_empty_parsedMailDict
):
    core.utils.mailParsing._parseMultipleHeader(
        mock_good_mailMessage, "test", mock_empty_parsedMailDict
...
def test__parseMultipleHeader_emptyMessage(
    mock_logger, mock_bad_mailMessage, mock_empty_parsedMailDict
):
    core.utils.mailParsing._parseMultipleHeader(
        mock_bad_mailMessage, "multi", mock_empty_parsedMailDict
    )
    assert "multi" in mock_empty_parsedMailDict
...
def test__parseMultipleHeader_noMessage(mock_no_mailMessage, mock_empty_parsedMailDict):
    with pytest.raises(AttributeError):
        core.utils.mailParsing._parseMultipleHeader(
            mock_no_mailMessage, "multi", mock_empty_parsedMailDict
        )
